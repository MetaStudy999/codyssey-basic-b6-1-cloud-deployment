# B6-1 작업 룰(Working Rules)

이 문서는 B6-1에서 사용하는 **미션별 작업 운영 어댑터(Mission Working Rules Adapter)**입니다. 공통 규칙 전문을 복제하지 않고 메인 레포(Control Tower)의 상위 표준을 사용합니다.

## 빠른 적용(Quick Apply)

```text
B6-1 공식 Mission / Evaluation / 제공 파일
→ 현재 B6-1 Repository main
...
```text
1. B6-1 공식 Mission / Evaluation / 제공 파일
...
비밀정보(Secret)는 값이 아니라 존재·경로·소유권·그룹·권한 등 메타데이터 중심으로 검증합니다. Cloud/API/AI 비용이 있는 경우 비용·자원 보호와 정리(Cleanup)를 실행 전에 확인합니다.
...
이 문서에는 B6-1 고유 예외만 추가하고, 공통 작업 룰 전문을 복제하지 않습니다.
